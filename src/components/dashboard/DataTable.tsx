  ...
  return (
    <div className="bg-white rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.02)] border border-gray-100 overflow-hidden group/table translate-y-0 hover:-translate-y-1 transition-all duration-500">
      <div className="p-10 border-b border-gray-50 flex items-center justify-between bg-white relative">
        ...
        <h3 className="text-3xl font-black text-[#0b3155] tracking-tighter">{title}</h3>
        ...
        <div className="flex items-center gap-6">
          <div className="relative group/search">
            <input 
              ...
              className="bg-gray-50/50 border border-gray-100 rounded-2xl py-3.5 pl-14 pr-6 text-xs font-black focus:ring-4 focus:ring-[#0b3155]/5 focus:bg-white focus:border-[#0b3155]/20 transition-all outline-none w-80"
            />
          ...
          <button className="p-4 bg-gray-50/50 border border-gray-100 rounded-2xl text-gray-400 hover:bg-white hover:shadow-xl transition-all group/filter">
      ...
      <div className="overflow-x-auto px-5 pb-5">
        <table className="w-full text-left border-separate border-spacing-y-3">
          <thead>
            <tr>
              {columns.map((col) => (
                <th key={col.key} className="px-8 py-6 text-[10px] font-black text-gray-300 uppercase tracking-[0.2em] first:pl-10">
                  {col.label}
                </th>
              ))}
          ...
          <tbody className="">
            {data.map((row, i) => (
              <tr key={i} className="group/row transition-all hover:scale-[1.01] duration-300">
                {columns.map((col) => (
                  <td key={col.key} className="px-8 py-8 text-sm font-black text-[#0b3155]/80 bg-gray-50/30 group-hover/row:bg-white group-hover/row:shadow-[0_10px_30px_rgba(0,0,0,0.03)] first:rounded-l-[24px] first:pl-10 last:rounded-r-[24px] transition-all">
                    {col.render ? col.render(row[col.key]) : row[col.key]}
                  </td>
                ))}
                <td className="px-8 py-8 text-right bg-gray-50/30 group-hover/row:bg-white group-hover/row:shadow-[0_10px_30px_rgba(0,0,0,0.03)] last:rounded-r-[24px] transition-all">
